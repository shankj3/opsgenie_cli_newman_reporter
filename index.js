...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('beforeItem', function (err, o) {
        ...
        if (typeof testResult !== 'undefined' && testResult !== null ) {
            if (!isEmptyObject(testResult) && testResult.assertion_errors_length > 0) {
                var opsGeniePost = {};
                opsGeniePost.message = "Failed Test: " + testResult.name
                opsGeniePost.alias = testResult.name
                opsGeniePost.description = "Assertion errors in tests against URI: " + testResult.uri + " \n    " + testResult.assertion_errors.join("    \n")
                opsGeniePost.entity = testResult.name
                opsGeniePost.source = "newman health check test runner"
                // TODO: send to opsgenie
                // console.log(JSON.stringify(opsGeniePost))
                var xhr = new XMLHttpRequest();
                var url = "https://api.opsgenie.com/v2/alerts";
                xhr.open("POST", url, true);
                xhr.setRequestHeader("Content-type", "application/json");
                // todo get from env
                xhr.setRequestHeader("Authorization", "GenieKey 7e340aea-f414-4c56-a02e-2176f14f16bb");
                ...
                var data = JSON.stringify(opsGeniePost);
                xhr.send(data);
            }
        }
        // clear result object
        testResult = {};
        testResult["assertion_errors"] = [];
        testResult["assertion_errors_length"] = 0;
        testResult.test_passed = true;
    ...
    emitter.on('beforeRequest', function (err, o) {
        if (err) { return; }
        testResult.uri = o.request.url.getPathWithQuery()
    ...
    emitter.on('assertion', function (err, o) {
        ...
        if (!passed) {
            testResult.name = o.item.name;
            testResult.assertion_errors.push(o.error.message);
            testResult.assertion_errors_length++
